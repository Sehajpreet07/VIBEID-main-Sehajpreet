...
def auto_enroll_dialog(subject_code):
    ...
    if check.data:
        ...
        if st.button('Got it!',key="already_enrolled_got_it"):
            st.query_params.clear()
            st.rerun()
        return
    ...
    with col1:
        if st.button('No thanks',key="enrolled_nothanks"):
            st.query_params.clear()
            st.rerun()
    with col2:
        if st.button('Yes enroll now!', type='primary', use_container_width=True, key="finallyenrolled"):
